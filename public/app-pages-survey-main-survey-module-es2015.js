...
let MainSurveyModule = class MainSurveyModule {
};
...
let SurveyComponent = class SurveyComponent {
    constructor(userDataService, surveyService, route, router, authService, formBuilder) {
    ...
    deleteSurvey(id) {
        ...
        sweetalert2__WEBPACK_IMPORTED_MODULE_8___default.a.fire({
            ...
            if (willDelete.value) {
                this.surveyService.deleteSurvey(id).subscribe((status) => {
                    let index = -1, count = 0;
                    this.surveyReports.forEach((r) => {
                    ...
                    if (index > -1) {
                        this.surveyReports.splice(index, 1);
                    }
    ...
    checkAll(ev) {
        this.surveyReports.forEach(x => x.selected = ev.target.checked);
    }
    isAllChecked() {
        return this.surveyReports.every(_ => _.selected);
    }
...
let ViewSurveyComponent = class ViewSurveyComponent {
    constructor(userDataService, formBuilder, oldSurveyService, route, router, surveyService, authService) {
        this.userDataService = userDataService;
        this.formBuilder = formBuilder;
    ...
    getReport(id) {
        this.loading = true;
        this.surveyService.getSurveyByID(id).subscribe((report) => {
            this.report = report;
            this.report.structure_ = JSON.parse(this.report.structure);
            console.log(this.report.structure_);
            if (this.report.comments !== undefined) {
                this.report.comments.forEach((comment) => {
                    const commentNameBadge = comment.commentor ? comment.commentor.full_name : 'AN';
                    const initials = commentNameBadge.match(/\b\w/g) || [];
            ...
            this.loading = false;
            if (this.report) {
                this.showApproval = !this.report.approved_by && _services_auth_service__WEBPACK_IMPORTED_MODULE_8__["AuthService"].can(this.permissions.approve_survey_report);
                this.showRequestApproval = !this.report.approved_by && !_services_auth_service__WEBPACK_IMPORTED_MODULE_8__["AuthService"].can(this.permissions.approve_survey_report) && this.report.request_approval;
                this.url = _app_constants__WEBPACK_IMPORTED_MODULE_4__["Constants"].url;
            }
        }, (err) => {
    ...
    onSubmit({ value, valid }) {
        ...
        this.errorMessage = '';
        this.successMessage = '';
        this.oldSurveyService.addComment(value).subscribe((resp) => {
            if (resp) {
                ...
                this.NewCommentForm.patchValue({ comment: '' });
            }
            else {
    ...
    approveSurvey(id) {
        sweetalert2__WEBPACK_IMPORTED_MODULE_10___default.a.fire({
            ...
            if (willUpdate.value) {
                this.surveyService.approveSurvey(id).subscribe(updated => {
    ...
    print() {
        const element = document.getElementById('printSection').innerHTML;
        console.log(element);
...
//# sourceMappingURL=app-pages-survey-main-survey-module-es2015.js.map