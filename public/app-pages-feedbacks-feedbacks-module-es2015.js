...
let FeedbackListComponent = class FeedbackListComponent {
    constructor(feedbacksService, route, _Location, router) {
    ...
    submitSearch($perPage = 15) {
        this.loading = true;
        this.feedbacks = [];
        this.last_page = -1;
        this.getFeedback('', this.searchText);
    }
    getFeedback(url, params = '') {
        this.feedbacksService.getPageFeedbacks(url, params).subscribe((data) => {
    ...
    deleteFeedback(feedback) {
        _app_constants__WEBPACK_IMPORTED_MODULE_4__["Constants"].confirm({
            ...
            if (willDelete.value) {
                this.feedbacksService.deleteFeedback(feedback.id).subscribe((status) => {
                    if (status) {
...
let ViewFeedbackComponent = class ViewFeedbackComponent {
    constructor(feedbacksService, formBuilder, route, router, _location) {
        this.feedbacksService = feedbacksService;
    ...
    getFeedback(id) {
        ...
        this.feedbacksService.getFeedbackDetail(id)
            ...
            for (let j = 0; j < this.feedback.responses.length; j++) {
                this.commentNameBadge = this.feedback.responses[j].responder ? this.feedback.responses[j].responder.full_name : 'Anonymous';
                const initials = this.commentNameBadge.match(/\b\w/g) || [];
                this.commentInitials = ((initials.shift() || '') + (initials.pop() || '')).toUpperCase();
                this.feedback.responses[j].commentInitials = this.commentInitials;
            }
...
//# sourceMappingURL=app-pages-feedbacks-feedbacks-module-es2015.js.map