...
let AddDocumentFormatComponent = class AddDocumentFormatComponent {
    constructor(formBuilder, router, route, documentFormatService) {
    ...
    onSubmit({ value, valid }) {
        if (value) {
            this.loading = true;
            this.documentFormatService.addDocFormat(value).subscribe((document_format) => {
                if (document_format) {
...
let DocumentFormatsComponent = class DocumentFormatsComponent {
    constructor(documentFormatService, route, _Location, router, formBuilder) {
        this.documentFormatService = documentFormatService;
        this.route = route;
    ...
    getDocFormats(filter = '') {
        this.fetching = true;
        this.doc_formats = [];
        this.documentFormatService.getDocumentFormats(filter).subscribe((doc_formats) => {
    ...
    getAllowedDocFormats() {
        this.loading = true;
        this.documentFormatService.getAllowedDocumentFormats().subscribe((allowdoc_formats) => {
    ...
    onChange(doc_format, isChecked) {
        const allowFormArray = this.NewAllowForm.controls.allowed;
        if (isChecked) {
    ...
    onSubmit({ value, valid }) {
        if (value) {
            this.loading = true;
            this.documentFormatService.allowDocFormat(value).subscribe((doc_formats) => {
                if (doc_formats) {
    ...
    deleteDocFormat(id) {
        sweetalert2__WEBPACK_IMPORTED_MODULE_7___default.a.fire({
            ...
            if (willDelete.value) {
                this.loading = true;
                this.documentFormatService.deleteDocumentFormat(id).subscribe((status) => {
                    if (status) {
...
let DocumentFormatsModule = class DocumentFormatsModule {
};
...
let EditDocumentFormatComponent = class EditDocumentFormatComponent {
    constructor(formBuilder, router, route, documentformatService) {
        this.formBuilder = formBuilder;
        this.router = router;
        this.route = route;
    ...
    getDocumentFormat(id) {
        ...
        this.documentformatService.getDocumentFormatByID(id).subscribe((doc_format) => {
    ...
    onSubmit({ value, valid }) {
        ...
        if (value.id) {
            this.loading = true;
            this.documentformatService.updateDocumentFormat(value).subscribe((doc_format) => {
                if (this.doc_format) {
...
let ViewDocumentFormatComponent = class ViewDocumentFormatComponent {
    constructor(documentFormatService, route, router, _location) {
        this.documentFormatService = documentFormatService;
        this.route = route;
        this.router = router;
        this._location = _location;
...
//# sourceMappingURL=app-pages-documents-document-formats-document-formats-module-es2015.js.map