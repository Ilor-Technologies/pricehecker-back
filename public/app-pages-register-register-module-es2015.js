...
let InternalUsersComponent = class InternalUsersComponent {
    constructor(router, userDataService, vendorDataService, gradeService, _Location, formBuilder) {
        this.router = router;
    ...
    getRegistrationData() {
        ...
        this.userDataService.getMDARegistrationData().subscribe((data) => {
    ...
    onSubmit({ value, valid }) {
        ...
        if (value) {
            this.userDataService.addUser(value)
                ...
                this.loading = false;
                _app_constants__WEBPACK_IMPORTED_MODULE_8__["Constants"].handleError(err, this.router, true);
            });
        }
    }
    checkEmail(email) {
        ...
        this.vendorDataService.IfExists(field, email)
            .subscribe((status) => {
            this.emailIsTaken = status;
            this.ereqLoading = false;
            this.emailMessage = 'This email is already exists';
        }, (err) => {
            this.errorMessage = _app_constants__WEBPACK_IMPORTED_MODULE_8__["Constants"].handleError(err, this.router);
        });
    }
    checkUsername(username) {
        ...
        this.vendorDataService.IfExists(field, username).subscribe((status) => {
            this.usernamelIsTaken = status;
            this.ureqLoading = false;
            this.usernameMessage = 'This username already exists';
        }, (err) => {
            this.errorMessage = _app_constants__WEBPACK_IMPORTED_MODULE_8__["Constants"].handleError(err, this.router);
        });
    }
    ...
    updateParastatalCode(id) {
        const institution = this.internalUserForm.get('institution_id').value;
        alert(institution);
        const institiondRec = this.institutions.find(e => e.id === institution);
        if (institiondRec) {
            this.selectedParastatal = institiondRec.parastatals.find(ee => ee.id === id);
        }
...
let VendorSignUpComponent = class VendorSignUpComponent {
    constructor(router, formBuilder, vendorDataService, _Location) {
        this.router = router;
        this.formBuilder = formBuilder;
        this.vendorDataService = vendorDataService;
        this._Location = _Location;
    ...
    hitBppApi(rcNumber) {
        ...
        if (rcNumber) {
            ...
            this.vendorDataService.fetchVendorDataFromBPPApi(rcNumber).subscribe((response) => {
                this.loadingFromApi = false;
                if (response.length) {
                    ...
                    this.hasConfirmedRCNumber = true;
                }
                else {
                    sweetalert2__WEBPACK_IMPORTED_MODULE_7___default.a.fire({
                        ...
                        if (goOn.value) {
                            this.justContinue = true;
                        }
                    });
                }
            }, (err) => {
                this.apiError = _app_constants__WEBPACK_IMPORTED_MODULE_6__["Constants"].handleError(err, this.router);
                this.loadingFromApi = false;
            });
        }
        else {
    ...
    onSubmit({ value, valid }) {
        this.loading = true;
        if (value) {
            this.vendorDataService.addVendor(value).subscribe((vendor) => {
                if (vendor) {
                    this.display = 'successful';
                }
                else {
    ...
    checkEmail(email) {
        ...
        this.ereqLoading = true;
        const field = 'email';
        this.vendorDataService.IfExists(field, email).subscribe((status) => {
            this.emailIsTaken = status;
            this.ereqLoading = false;
            this.emailMessage = 'This email already exists';
        }, (err) => {
            this.emailMessage = _app_constants__WEBPACK_IMPORTED_MODULE_6__["Constants"].handleError(err, this.router);
        });
    }
    checkUsername(username) {
        username = username.trim();
        ...
        this.vendorDataService.IfExists(field, username).subscribe((status) => {
            ...
            this.usernameMessage = _app_constants__WEBPACK_IMPORTED_MODULE_6__["Constants"].handleError(err, this.router);
        });
    }
    checkPhone(phone) {
        this.phoneMessage = '';
        ...
        const field = 'phone';
        this.vendorDataService.IfExists(field, phone).subscribe((status) => {
            this.phoneIsTaken = status;
            this.preqLoading = false;
            this.phoneMessage = 'This phone number already exists';
        }, (err) => {
            this.phoneMessage = _app_constants__WEBPACK_IMPORTED_MODULE_6__["Constants"].handleError(err, this.router);
        });
    }
    checkRCNumber(rc_number) {
        this.rcMessage = '';
        if (rc_number === undefined || rc_number.toString().trim() === '') {
            return;
        }
        this.rcreqLoading = true;
        const field = 'rc_number';
        this.vendorDataService.IfExists(field, rc_number)
            .subscribe((status) => {
            this.rclIsTaken = status;
            this.rcreqLoading = false;
            this.rcMessage = 'This RC Number already exists';
        }, (err) => {
            this.rcMessage = _app_constants__WEBPACK_IMPORTED_MODULE_6__["Constants"].handleError(err, this.router);
        });
    }
    checkTaxID(tax_id) {
        tax_id = tax_id.trim();
        ...
        const field = 'tax_identification_number';
        this.vendorDataService.IfExists(field, tax_id).subscribe((status) => {
            this.taxlIsTaken = status;
            this.treqLoading = false;
            this.taxMessage = 'This tax identification number already exists';
        }, (err) => {
            this.taxMessage = _app_constants__WEBPACK_IMPORTED_MODULE_6__["Constants"].handleError(err, this.router);
        });
    }
    showPassword(input) {
        input.type = input.type === 'password' ? 'text' : 'password';
...
//# sourceMappingURL=app-pages-register-register-module-es2015.js.map