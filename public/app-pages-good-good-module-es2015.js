...
let CreateGoodsComponent = class CreateGoodsComponent {
    constructor(formBuilder, router, route, goodsService, changeDetectorRef, elem, http, authService) {
    ...
    prepareSave() {
        const input = this.newForm.value;
    ...
    getCategories() {
        this.goodsService.getGoodsCategory().subscribe((categories) => {
            ...
            _app_constants__WEBPACK_IMPORTED_MODULE_7__["Constants"].handleError(err, this.router, true);
        });
    }
    onSubmit() {
        ...
        this.saving = true;
        this.errorMessage = '';
        this.goodsService.addNewGood(formModel).subscribe((res) => {
            this.newForm.reset();
            this.properties = [];
            sweetalert2__WEBPACK_IMPORTED_MODULE_8___default.a.fire('Successful!', res.message, 'success');
            this.saving = false;
        }, (error) => {
            _app_constants__WEBPACK_IMPORTED_MODULE_7__["Constants"].handleError(error, this.router, true);
            this.saving = false;
        });
    }
    addNewProperty() {
...
let EditGoodsComponent = class EditGoodsComponent {
    constructor(formBuilder, router, route, changeDetectorRef, elem, http, authService, goodsService) {
        this.formBuilder = formBuilder;
        this.router = router;
        this.route = route;
        this.changeDetectorRef = changeDetectorRef;
    ...
    fetchDetail(id) {
        this.loading = true;
        this.goodsService.getGoodsDetail(id).subscribe((good) => {
            this.good = good;
            this.editForm.get('name').setValue(this.good.name);
            this.editForm.get('category_id').setValue(this.good.category_id);
            this.editForm.get('short_description').setValue(this.good.short_description);
            this.properties = [];
            if (this.good.properties_template) {
                this.good.properties_template.forEach(property => {
    ...
    getCategories() {
        this.goodsService.getGoodsCategory().subscribe((categories) => this.categories = categories);
    }
    addNewProperty() {
        this.properties.push({ name: '', type: '' });
    }
    ...
    onSubmit() {
        ...
        this.saving = true;
        this.errorMessage = '';
        this.goodsService.updateGoods(formModel, this.good.id).subscribe((res) => {
    ...
    onApprove() {
        ...
        sweetalert2__WEBPACK_IMPORTED_MODULE_3___default.a.fire({
            ...
            if (willDelete.value) {
                this.updating = true;
                this.errorMessage = '';
                this.goodsService.approveGoods(this.good.id).subscribe((res) => {
                    sweetalert2__WEBPACK_IMPORTED_MODULE_3___default.a.fire('Successful!', res.message, 'success');
                    this.good = res.data;
                    this.updating = false;
                }, (err) => {
...
// declare var $: any;
let GoodsListComponent = class GoodsListComponent {
    constructor(formBuilder, goodsService, route, router, http, authService) {
        this.formBuilder = formBuilder;
        this.goodsService = goodsService;
        this.route = route;
        this.router = router;
    ...
    get hasSomeItemsChecked() {
        return this.goods.filter(x => x.selected).length;
    }
    checkAll(ev) {
...
//# sourceMappingURL=app-pages-good-good-module-es2015.js.map