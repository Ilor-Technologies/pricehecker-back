...
let ChatMessageComponent = class ChatMessageComponent {
    constructor(uploadService, chatService, router) {
    ...
    get filteredGroups() {
        return this.chatService.groups.filter(ins => {
            return (ins.group.name.toString().toLowerCase().includes(this.searchText.toLowerCase())
                && ins.group.id !== this.chat.group.id);
        });
    }
    openMedia(mes) {
        const media = this.mediaMessages;
        const selection = media.findIndex(m => m.id === mes.id);
        const index = selection > -1 ? selection : 0;
        if (index < 0)
            return;
        this.lightBox.open(index);
    }
    get mediaMessages() {
        return this.chat.sortedMessages.filter(mes => {
...
/* harmony import */ var ngx_timeago__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ngx-timeago */ "./node_modules/ngx-timeago/fesm2015/ngx-timeago.js");
/* harmony import */ var _ctrl_ngx_emoji_mart__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(/*! @ctrl/ngx-emoji-mart */ "./node_modules/@ctrl/ngx-emoji-mart/fesm2015/ctrl-ngx-emoji-mart.js");
...
let ChatComponent = class ChatComponent {
    constructor(authService, formBuilder, router, chatService) {
        ...
        this.errorMessage = '';
        this._this = this;
        this.subject.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_9__["debounceTime"])(500), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_9__["distinctUntilChanged"])()).subscribe(newValue => {
            this.searchText = newValue;
        });
        this.groupSubjectFilter.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_9__["debounceTime"])(500), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_9__["distinctUntilChanged"])()).subscribe(newValue => {
            this.groupFilterText = newValue;
        });
    }
    get searchedUsers() {
    ...
    get filteredChatGroup() {
        return this.chatInstances.filter(group => group.group.name.toString().toLowerCase().indexOf(this.groupFilterText.toLowerCase()) > -1).sort((b, a) => {
            const dateA = a.last_message ? a.last_message.time : a.group.time, dateB = b.last_message ? b.last_message.time : b.group.time;
    ...
    get chatList() {
        return this.chatService.groups.filter(group => group.isInit);
    }
    get vendors() {
        return this.searchedUsers.filter(user => {
            const searchText = this.searchText.toLowerCase();
            return (user.is_vendor && !this.selectedVendors.includes(user) &&
    ...
    get users() {
        return this.searchedUsers.filter(user => {
            return (!user.is_vendor && !this.selectedUsers.includes(user) &&
                (user.full_name.toLowerCase().indexOf(this.searchText.toLowerCase()) > -1));
        }).sort((a, b) => {
    ...
    get selectedVendors() {
        return this.selecteds.filter(user => user.is_vendor).sort((a, b) => {
            const aName = (a.vendor ? a.vendor.business_name : a.full_name).toLowerCase(), bName = (b.vendor ? b.vendor.business_name : b.full_name).toLowerCase();
            if (aName > bName) {
                return 1;
            }
    ...
    extractLastMessage(group) {
        if (!group.last_message) {
            return group.group.time_created;
        }
        const lastMessage = group.last_message, doc = document.createElement('div');
        ...
        let creator = lastMessage.was_sent ? '<i class="ion ion-md-checkmark"></i>' : '<i class="ion ion-md-time"></i>';
        if ((this.chatService.presentUser || { i: '' }).i.toString() !== (group.last_message || { by: '' }).by.toString()) {
            const senders = group.group.participants.filter(user => user.id.toString() === group.last_message.by);
            if (senders.length) {
    ...
    createConversation(form) {
        ...
        sweetalert2__WEBPACK_IMPORTED_MODULE_6___default.a.fire({
            ...
            if (goOn.value) {
                ...
                const d = { participants: [], description: description, signature: uuid__WEBPACK_IMPORTED_MODULE_11__["v4"]() };
                this.selecteds.forEach(user => d.participants.push(user.id));
                this.chatService.createGroup(d).subscribe((data) => {
                    ...
                    this.loadingSearch = false;
                    const sels = this.selecteds;
                    this.selecteds = [];
                    sels.forEach(sel => {
                        this.searchedUsers.push(sel);
                    });
                    this.createConversationModal.display = 1;
                    document.getElementById('close-modal-button').click();
                    this.creatingGroup = false;
                }, (err) => {
                    this.errorMessage = _app_constants__WEBPACK_IMPORTED_MODULE_4__["Constants"].handleError(err, this.router, true);
                    this.creatingGroup = false;
                });
            }
        });
...
//# sourceMappingURL=app-pages-chat-chat-module-es2015.js.map