...
let CreateRoleComponent = class CreateRoleComponent {
    constructor(formBuilder, rolesService, router) {
    ...
    onChange(permission, isChecked) {
        const index = this.rolePermission.indexOf(permission);
        if (index > -1) {
    ...
    onSubmit() {
        ...
        this.loading = true;
        if (v.name) {
            this.rolesService.insertRole(v).subscribe((role) => {
                if (role) {
                ...
                this.loading = false;
                sweetalert2__WEBPACK_IMPORTED_MODULE_3___default.a.fire('Error', this.errorMessage, 'error');
            });
        }
    }
    ...
    get filteredPermissions() {
        return this.permissions.filter(permission => permission.toLowerCase().indexOf(this.permissionFilter.toLowerCase()) > -1);
    }
    getPermissions() {
...
let EditRoleComponent = class EditRoleComponent {
    constructor(router, route, rolesService, formBuilder) {
        this.router = router;
        this.route = route;
        this.rolesService = rolesService;
        this.formBuilder = formBuilder;
    ...
    get filteredPermissionList() {
        return this.allPermissions.filter(permission => permission.toLowerCase().indexOf(this.permissionFilter.toLowerCase()) > -1);
    }
    getRolesDetail(name) {
        this.pageLoading = true;
        this.rolesService.getRole(name).subscribe((result) => {
            this.role = result.role;
            this.rolePermission = result.permissions;
            this.allPermissions = result.all_permissions;
            this.pageLoading = false;
    ...
    onSubmit() {
        ...
        this.loading = true;
        if (v.name) {
            this.rolesService.updateRole(this.original_name, v).subscribe((role) => {
                if (role) {
...
let RolesListComponent = class RolesListComponent {
    constructor(rolesService, route, router) {
        this.rolesService = rolesService;
        this.route = route;
        this.router = router;
        this.roles = [];
    ...
    getRoles(value = '') {
        this.loading = true;
        this.roles = [];
        this.rolesService.getRoles(value).subscribe((roles) => {
            this.roles = roles;
            this.loading = false;
        }, (err) => {
            this.errorMessage = _app_constants__WEBPACK_IMPORTED_MODULE_2__["Constants"].handleError(err, this.router);
    ...
    deleteRole(role, i) {
        sweetalert2__WEBPACK_IMPORTED_MODULE_3___default.a.fire({
            ...
            if (willDelete.value) {
                this.rolesService.deleteRole(role.name).subscribe((status) => {
                    if (status) {
    ...
    checkAll(ev) {
        if (this.roles) {
            this.roles.forEach(x => x.selected = ev.target.checked);
        }
    }
    isAllChecked() {
        if (this.roles) {
            return this.roles.every(_ => _.selected);
        }
        return false;
    }
};
...
//# sourceMappingURL=app-pages-roles-roles-module-es2015.js.map