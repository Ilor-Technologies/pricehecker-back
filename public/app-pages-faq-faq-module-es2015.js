...
let AddFaqComponent = class AddFaqComponent {
    ...
    onSubmit({ value, valid }) {
        this.loading = true;
        if (value) {
            this.faqService.addFaq(value).subscribe((faqs) => {
                if (faqs) {
...
let EditFaqComponent = class EditFaqComponent {
    ...
    onSubmit({ value, valid }) {
        ...
        this.loading = true;
        if (value.id) {
            this.faqService.updateFaq(value).subscribe((faq) => {
                this.loading = false;
                if (this.faq) {
    ...
    onDelete(event) {
        ...
        _app_constants__WEBPACK_IMPORTED_MODULE_4__["Constants"].confirm({ message: 'Do you want to delete this record?', confirmText: 'Delete' }, (willDelete) => {
            if (willDelete.value) {
                this.deleteMessageEnabled = true;
                this.faqService.deleteFaq(this.faq.id).subscribe((status) => {
                    if (status) {
...
let FAQComponent = class FAQComponent {
    constructor(faqService, route, _Location, router) {
        this.faqService = faqService;
        this.route = route;
    ...
    loadData(search) {
        ...
        this.faqService.getAllFaqs(search).subscribe((faqs) => {
            this.faqs = faqs;
            if (!faqs.length) {
                this.placeHolderMessage = 'No FAQ On The System Yet!';
            }
            else {
                this.placeHolderMessage = '';
            }
    ...
    deleteFaq(id) {
        sweetalert2__WEBPACK_IMPORTED_MODULE_6___default.a.fire({
            ...
            if (willDelete.value) {
                this.faqService.deleteFaq(id).subscribe((status) => {
                    if (status) {
                        ...
                        this.errorMessage = 'Unable to delete Faq';
                    }
                }, (err) => {
                    _app_constants__WEBPACK_IMPORTED_MODULE_5__["Constants"].handleError(err, this.router, true);
                });
...
//# sourceMappingURL=app-pages-faq-faq-module-es2015.js.map