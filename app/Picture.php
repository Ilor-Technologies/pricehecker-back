...
class Picture extends BaseModel
{
    ...
    protected $fillable = ['id', 'path', 'mime_type', 'seo_filename', 'is_new'];
    ...
    public function getPathAttribute() {
        return route('picture.path', [$this->attributes['path']]);
    }
...
//    }
    ...
    */
     ...
     * @return array
     */
    public static function getPicture($request, $file = 'picture')
    {
     ...
     * @return bool|Picture
     */
    protected function _saveFile(UploadedFile $picture, $isImage = true)
    {
        //save picture to disk /time()/
        $dir = $this->getPictureDirectory(null);
        if ($path = $picture->store($dir)) {
            ...
            if ($isImage) {
                ...
                $chunks = explode('/', $path);
                $fileName = end($chunks);
                $picture->move(storage_path() . '/app/' . $dir, $fileName);
            }

            $data = [
                'path' => $path,
                'mime_type' => $picture->getClientMimeType(),
                'seo_filename' => $this->getOriginalUploadedFileName($picture, null)
            ];
            //persist to db
