...
class AdminActionsSubscriber {
    ...
    public function onInstitutionUpdated($event) {
        switch ($event->action) {
            case -1://delete
                EventController::save('Institution details updated', 2, 'Institution identified with code: ' . $event->institution->code . ' was deleted by user with ID: ' . $event->admin->id, __FILE__);
                break;
            case 0:
                EventController::save('Institution registration', 2, 'Institution identified with code: ' . $event->institution->code . ' was created by user with ID: ' . $event->admin->id, __FILE__);
                break;
            case 1://update
                EventController::save('Institution deleted', 2, 'Details of Institution  with ID: ' . $event->institution->id . ' was updated by user with ID: ' . $event->admin->id, __FILE__);
                break;
        }
    }
